...
int main() {
 test("HTTP request budget stops cancellation and slow trickles",[]{RequestBudget b{1000};CHECK(b.permits(20999,true));CHECK(!b.permits(21000,true));CHECK(!b.permits(1001,false));CHECK(!b.permits(40000,true));});
 ...
 test("exhausted changed cadence retries instead of going dormant",[]{Coordinator c;c.records[2]={10000,11800,0,11000,false,false};c.activate(11790,true);c.takeDue(11790,true);for(int i=0;i<3;++i)c.complete(Kind(i),11790,true);c.reconcile(11800);CHECK(c.takeDue(11820,true)==4);bool accepted=acceptRecord(Kind::Legion,c.records[2],{10000,11800,0,11820,false,false});CHECK(!accepted);c.complete(Kind::Legion,11820,accepted);CHECK(c.takeDue(11879,true)==0);CHECK(c.takeDue(11880,true)==4);});
 ...
 test("clock jump schedules one reconciliation",[]{Coordinator c;CHECK(!c.observeClock(100,1000));CHECK(!c.observeClock(101,2000));CHECK(c.observeClock(701,3000));CHECK(c.takeDue(701,true)==7);for(int i=0;i<3;++i)c.complete(Kind(i),701,true);CHECK(!c.observeClock(702,4000));CHECK(c.takeDue(702,true)==0);});
 test("clock rewind preserves elapsed retry wait",[]{Coordinator c;c.observeClock(10000,1000);c.activate(10000,true);c.takeDue(10000,true);for(int i=0;i<3;++i)c.complete(Kind(i),10000,false);CHECK(c.observeClock(6401,2000));CHECK(c.takeDue(6459,true)==0);CHECK(c.takeDue(6460,true)==7);});
 ...
 test("larger text expands both label and countdown columns",[]{auto l=layout({22,270,85},false);CHECK(l.timeWidth>=175&&l.labelWidth>=120);CHECK(58+l.labelWidth+8+l.timeWidth+8<=l.width);});
