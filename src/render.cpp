...
bool Renderer::draw(HWND hwnd,Preferences p,bool settings,const std::array<Record,3>& records,Time now,float scale,Box bounds,const std::wstring& status){
 ...
 if(settings){
  ...
  if(!p.collapsed){const wchar_t* names[]={L"World Boss",L"Helltide",L"Legion"};unsigned colors[]={0xdfa89d,0xf2b566,0xa7bfca};
   for(int i=0;i<3;++i){float y=4.f+i*l.row;auto v=calculate(Kind(i),records[i],now);icon(i,42,y+l.row/2.f,colors[i]);text(names[i],58,y,static_cast<float>(l.labelWidth),static_cast<float>(l.row),static_cast<float>(p.font),0xe3e6ed);
    auto value=formatTime(v);if(i==1)value=(v.active?L"ends ":L"in ")+value;
    text(value,w-l.timeWidth-8,y,static_cast<float>(l.timeWidth),static_cast<float>(l.row),static_cast<float>(p.font),v.estimated?0xa8b2c3:colors[i],true,true);
   }
  }
 }
